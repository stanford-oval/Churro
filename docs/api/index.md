# API Reference

The pages in this section are generated from the owning modules for each public API surface. Use them when you need exact class signatures, field definitions, and helper function behavior.

Most readers should start with the page that matches the task they are working on:

- [`churro_ocr`](churro_ocr.md) for the top-level convenience imports.
- [`churro_ocr.document`](document.md) for document OCR pipelines and result types.
- [`churro_ocr.ocr`](ocr.md) for single-image OCR entry points.
- [`churro_ocr.page_detection`](page_detection.md) for crop extraction and detection requests.
- [`Provider APIs`](providers.md) for backend specs, provider builders, and detector backends.
- [`Template APIs`](templates.md) and [`churro_ocr.prompts`](prompts.md) for advanced customization.

```{toctree}
:maxdepth: 1

churro_ocr
document
ocr
page_detection
providers
templates
prompts
```
