...
For `zai-org/GLM-OCR`, the built-in `hf` and `openai-compatible` backends both use the model's documented `Text Recognition:` prompt so local Transformers inference and served vLLM inference stay aligned.
